...
function buyPrestige() {
    // Attempts to buy a prestige row.
    /* 
        new plan: player can spend large amount of data to restart game with one sentence decoded and a way to autoBuy last item.
    */
    if (gameData.dataHacked >= prestigeCost() && allowedSentances()) {
        subtractData(prestigeCost());
        while (prestigePrint());
        HTMLEditor('prestigeCost', formatBytes(prestigeCost())); // Updates cost UI.
    }
}

function allowedSentances() {
    let allowedSentances = 1;
    if (prestige.data.sentancesDisplayed < allowedSentances) {
        return true;
...
function prestigeCost() {
    // Calculates the cost to unlock the next row of the prestige message.
    // Rows within a sentence will be similar in cost.
    // Each sentence will have a substantial cost difference.
    let rows = prestige.data.rowsDisplayed + 1;
    let sents = prestige.data.sentancesDisplayed + 1;
    let cost = (rows * sents) * (Math.pow(100, sents));
...
function prestigePrint() {
    ...
    if (prestige.data.sentancesDisplayed < prestige.display.message.length) { // If there are sentences left to display.
        ...
        if (prestige.data.rowsDisplayed >= prestige.display.message[prestige.data.sentancesDisplayed].length) { // If the displayed row is the last row in the sentence.
            ...
            prestige.data.rowsDisplayed = 0; // The first row of the next sentence.
            return false;
        }
        else {
            return true;
